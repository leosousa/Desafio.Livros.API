...
public class ServicoListagemAutor : Notifiable<Notification>, IServicoListagemAutor
{
    ...
    public async Task<ListaPaginadaResult<AutorComLivroDto>> ListarComLivrosAsync(AutorListaFiltro filtros, int numeroPagina = 1, int tamanhoPagina = 10)
    {
        var predicado = PredicateBuilder.New<Entidades.Autor>(true);
        ...
        // para sabermos quantos registros pular internamente na busca, por isso enviamos
        // numeroPagina - 1
        var autores = await _repositorioAutor.ListarComLivrosAssociadosAsync(predicado, numeroPagina - 1, tamanhoPagina, e => e.Nome);

        var totalPaginas = quantidadeProdutos / tamanhoPagina;

        var result = new ListaPaginadaResult<AutorComLivroDto>
        {
            Itens = autores,
            NumeroPagina = numeroPagina,
            TamanhoPagina = tamanhoPagina,
